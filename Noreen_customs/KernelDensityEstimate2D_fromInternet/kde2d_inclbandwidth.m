function [bandwidth,density,X,Y]=kde2d(data,n,bw_factor1, bw_factor2,  MIN_XY,MAX_XY)
% same function as kde2d.m but allows for manual manipulation of bandwidth.
% bandwidth (sigma of gaussian) is rescaled by bw_factor 1 resp. 2.
% bandwidth=bandwidth_automatic*(bw_factor1,bw_factor2)
% i.e. if =1: no rescaling
% t_y & t_x in script must be adjusted by squared bw_factors
%
% ***************************************************************
% original: 
% ***************************************************************
...
if nargin<3 %(no bw_factor given. NW)
    bw_factor1=1;
    bw_factor2=1;
end
N=size(data,1);
if nargin<5  %(no Min/Max given) used to be: 3 (NW)
    MAX=max(data,[],1); MIN=min(data,[],1); Range=MAX-MIN;
    MAX_XY=MAX+Range/4; MIN_XY=MIN-Range/4;
end
...
% ****
% rescale the squared bandwidth (NW2014-12)
% see also: http://www.mathworks.com/matlabcentral/fileexchange/17204-kernel-density-estimation
t_y=t_y*bw_factor2^2;
t_x=t_x*bw_factor1^2;
% ****
