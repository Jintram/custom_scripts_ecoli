function  [Lout,OKorNot,quit_now,dontsave,addtolist,crop_pop,newrect,savetemp,backwards,gotoframenum,DJK_settings] = ...
    BleachData_PN_manual_kant(p,Lin,L_prec,phin,rect,rect_prec,phsub,DJK_settings,assistedCorrection)

% **************************************************************************
% NW2015-07-10. QUITE A DUMMY VERSION OF PN_MANUAL_KANT WITH NO MANUAL
% CORRECTION. WORKS WHEN CELLS ARE AUTOMATICCALY SEGMENTED CORRECTLY.
% 1) REMOVES DIRT ('b')
% 2) GOES TO NEXT IMAGE ('spacebar')
% RUN FROM 2nd FRAME ON
% The changes are marked with "blubb"
% **************************************************************************
...
% ***blubb.***
% Enforce input: 1) 'b', 2) 'spacebar'
AUTOMATICINPUTCOUNTER=0; % count which input is required. (1) or (2)
...
while ~done
    ...
    if ~ishandle(ourfig) % if segmented image figure doesn't exist yet (should never be the case)
        ...
        %stop3=toc
                ...
                % ***blubb***: try to not show images
        if assistedCorrection && ~isempty(L_prec)
  %blubb          Lshow = PN_imshowlabel(Lout,rect,L_prec,rect_prec,'phaseImage',phsub); %slow step 0.45sec! (NW 2012-05-10)
            %stop4a=toc %PN_imshow.. is slow step (0.45 sec)
  %blubb          Lshow = imresize_old(Lshow,res);
            %stop5a=toc
  %blubb         figure(ourfig) % NW2012-05-10
            %clf reset;% NW2012-05-10
  %blubb        imshow(Lshow,'InitialMagnification','fit');
 ...
 % *** OLD ***
  ...
  %  *** OLD ***
    ...
    % ****blubb***
    if AUTOMATICINPUTCOUNTER==0
        ct=1; %mouse (0) or character (1).
        cc='b'; %letter
    else 
        ct=1;
        cc=' ';
     ...
     % *** 
       ...
       elseif cc=='b'   % restrict segmentation to overlaps with previous segmentation image (useful for non-growing cells)
            ...
            if ~isempty(L_prec) % previous image exists (can only be loaded in asisted correction mode)
                ...
                L_prec_mask=imerode(L_prec_full,strel('disk',5)); % enlargen or make smaller  prev. image overlap mask if needed
