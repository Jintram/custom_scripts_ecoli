...
SCRIPTNAME = 'script20160920_riboSwitch_L19pn25.m';
...
clear schnitzcells; clear s_rm;

TITLE = 'prrsa-mCerulean_pn25-yfp'
THESHIFTTIME = 119; % Note 221 mins is what you'd manually calculate.
% timeofswitch = datenum(2016,02,17,16,23,00); schnitzstarttime = min([schnitzcells.timestamp]); switchedafterdays=timeofswitch-schnitzstarttime; switchedafterminutes=switchedafterdays*24*60
EXPERIMENTALID = ['2016-09-20_pos1_' TITLE];
GROWTHFIELD = 'muP9_fitNew_all';
load('U:\ZZ_EXPERIMENTAL_DATA\A_Step5_Data_per_project\Ribosomes\schnitzcell_datafiles\2016-09-20_pos1crop-Schnitz.mat');
subdir = [EXPERIMENTALID '\']
...
THESHIFTTIME = 119; % Note 221 mins is what you'd manually calculate.
% timeofswitch = datenum(2016,02,17,16,23,00); schnitzstarttime = min([schnitzcells.timestamp]); switchedafterdays=timeofswitch-schnitzstarttime; switchedafterminutes=switchedafterdays*24*60
EXPERIMENTALID = ['2016-09-20_pos2_' TITLE];
GROWTHFIELD = 'muP9_fitNew_all';
load('U:\ZZ_EXPERIMENTAL_DATA\A_Step5_Data_per_project\Ribosomes\schnitzcell_datafiles\2016-09-20_pos2crop-Schnitz.mat');
subdir = [EXPERIMENTALID '\']
...
THESHIFTTIME = 119; % Note 221 mins is what you'd manually calculate.
% timeofswitch = datenum(2016,02,17,16,23,00); schnitzstarttime = min([schnitzcells.timestamp]); switchedafterdays=timeofswitch-schnitzstarttime; switchedafterminutes=switchedafterdays*24*60
EXPERIMENTALID = ['2016-09-20_pos3_' TITLE];
GROWTHFIELD = 'muP9_fitNew_all';
load('U:\ZZ_EXPERIMENTAL_DATA\A_Step5_Data_per_project\Ribosomes\schnitzcell_datafiles\2016-09-20_pos3crop-Schnitz.mat');
subdir = [EXPERIMENTALID '\']

%% If necessary
if ~exist('schnitzcells') & exist('s_rm')
    schnitzcells = s_rm;
end

%% Parameters
OUTPUTDIR = 'D:\Local_Software\Martijn_extensions\Martijn_custom\singlecell_scriptsperexperiment\plots20160920\';
...
for i = 1:numel(growthtimesPerframe_nrs)    
    ...
    plot(growthtimesPerframe_nrs{i}(1),meangrowth(i),'.k','MarkerSize',10); 
    %errorbar(growthtimesPerframe_nrs{i}(1),meangrowth(i),stdgrowth(i),'k.'); 
end

xlabel('Time');
ylabel('Single cell growth rate [dbl/hr]');
